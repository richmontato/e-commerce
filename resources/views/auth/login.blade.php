    ...
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="mt-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div class="text-sm text-gray-600">
                ...
                <a href="{{ route('register') }}" class="block font-semibold text-indigo-600 hover:text-indigo-500 mt-1">
                    {{ __('Daftar sekarang') }}
                </a>
            </div>

            <x-primary-button class="w-full sm:w-auto normal-case text-sm px-6 py-3">
                {{ __('Login') }}
            </x-primary-button>
        </div>
    </form>
